...
def create_app(test_config=None):
    ...
    def CollectQuestions():
        ...
        # get the paginated Records from our paginatepages Function for
        # Questions
        ...
        # we dont care about pagination now we
        # will just send all available categories
    ...
    def deleteQuestion(questionid):
        try:
            ...
            Getquestion = Question.query.filter(
    ...
    def searchquestions():
        ...
        try:
            searchRes = Question.query.filter(
    ...
    def GetQuestionsFromCategories(catid):
        ...
        try:
            QuestionsFromCategory = Question.query.filter_by(
                category=Getcategory.id).order_by(Question.id).all()
            paginatedQuestions = paginatePages(
                request, QuestionsFromCategory, "Q")
    ...
    def playquizzes():
        try:
            ...
            GetQuizzQuestions = Question.query.filter(
                Question.id.notin_(previousQ)).filter_by(
                category=GetCategory.id).all()
            print(GetQuizzQuestions)
            # now we should excpect that
            # we have some questions that is not equal to
